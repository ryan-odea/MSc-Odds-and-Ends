...
set.seed(1)
nTotSamp <- dim(Auto)[1]
train <- sample(nTotSamp,round(nTotSamp/2))
```
...
for loop first is the standard k-fold CV estimate, as in (5.3). The second is a bias-corrected version. On this data set, the two estimates are very similar to each other.
...
```{r}
boot_out <- boot(Portfolio,alpha.fn,R=1000)
boot_out
```
...
sd(boot_out$t)
```
The final output shows that using the original data, $\hat{\alpha} = 0.5758$, and that the bootstrap estimate for $SE(\hat{\alpha} )$ is 0.0886. The output has two interesting elements. \$t contains the estimates of statistic generated by each bootstrap sample. \$t0 is the estimate caculated by the full original dataset.

```{r}
boot.ci(boot_out, conf=.95, type="norm")
```

We can also obtain the confidence interval for the statistic of interest from the boostrap result by boot.ci() function. The default gives a 95% confidence interval and all of five different confidence intervals. Among the five confidence intervals, studentized interval requires bootstrap variance which can be obtained with second-level bootstrap. 
...
```{r}
aa=boot(Auto,boot.fn,1000)
aa
```

This indicates that the bootstrap estimate for $SE(\hat{\beta}_{0})$ is 0.89, and that the bootstrap estimate for $SE(\hat{\beta}_{1})$ is 0.0077. As discussed in Section 3.1.2,
...
We can also do the residual bootstrap by using the Boot() function from the car package. Boot() function is specifically for bootstrapping for regression models. The following is an example of residual bootstrap on regression on the same dataset:
```{r}
lm.fit <- lm(mpg~horsepower,data=Auto)
Boot(lm.fit, R=1000, method = "residual")
```